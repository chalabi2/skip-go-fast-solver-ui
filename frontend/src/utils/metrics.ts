...
export const CHAIN_CONFIGS: ChainConfig[] = [
  ...
  },
  {
    domain: 10,
    name: "Optimism",
    rpcUrl: "https://op.chandrastation.com",
...
async function getSettlementDetailsForChain(
  ...
  try {
    ...
    // Special handling for Polygon only now
    const batchSize = chainConfig.name === "Polygon" ? 5 : chainOrders.length;
    ...
    for (let i = 0; i < chainOrders.length; i += batchSize) {
      ...
      // Add delay only for Polygon now
      if (chainConfig.name === "Polygon") {
